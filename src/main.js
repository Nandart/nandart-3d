...
const preloadTextures = async () => {
  ...
  const loadTexture = (path) => new Promise(resolve => {
    loader.load(path, resolve, undefined, (err) => {
      ...
      fallbackTexture.image.src = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z/C/HgAGgwJ/lK3Q6wAAAABJRU5ErkJggg==';
...
const closeArtworkDetails = () => {
  if (!artworkClone || !isFocusMode) return;
...
const init = async () => {
  try {
    ...
    alert('Ocorreu um erro ao carregar a galeria. Por favor, recarregue a página.');
  }
};
